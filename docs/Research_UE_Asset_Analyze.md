...
핵심: **Step N.5 의 제목은 Step N 과 동일한 자산 주제** 를 따른다. "Monolith X.Y 로 본" 같이 도구를 제목으로 두지 않는다. Step N.5 가 Step N 보다 더 상세한 이유 (도구 발전) 는 lead 또는 섹션 1 의 한 단락으로만 언급하고, 본문 8 섹션은 자산 분석 자체에 집중한다.
...
| 섹션 1 가 "두 축" (자산 축 + 도구 축) 으로 나뉘어 도구 축이 사실상 본문을 지배 | 자산 분석이 한 줄 요약으로만 남고, 도구 자랑이 자리를 차지 | 축은 자산 분석 하나. 도구 차이는 섹션 1 의 한 단락 배경 노트 |
