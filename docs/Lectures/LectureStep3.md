...
4 방향 (전 / 후 / 좌 / 우) 로코모션과 히스테리시스 기반 안정 방향 판정, 그리고 데이터 주도 디버그. 다음 같은 프로젝트에서 효과가 크다.

### 시점과 이동 방향이 분리되는 게임

- **3 인칭 슈터 / TPS**: 캐릭터가 적을 바라보면서 좌우로 스트레이프, 뒤로 후퇴. 4 방향 시퀀스가 필수.
- **락온 / 타겟팅 전투 시스템** (다크 소울, 베요네타 류, 몬스터 헌터 등): 적을 록 온한 채 8 방향 이동. 본 Step 의 4 방향에 대각선 또는 BlendSpace 보강을 더한 형태.
- **MOBA / RTS 의 캐릭터 뷰**: 카메라가 고정되어 캐릭터가 모든 방향으로 이동.
- **VR 게임의 외부 시점 캐릭터 표현**: 플레이어의 임의 이동 방향에 캐릭터 본을 맞춰야 함.
- **격투 게임의 백 스텝 / 사이드 스텝**: 정면 / 후면 / 좌 / 우 분리된 시퀀스 풀.

### 디자이너 협업이 잦은 팀 프로젝트

- **디자이너가 디버그 시각화를 자주 켜고 끌** 필요가 있는 팀: `S_DebugSetting` 의 bool 토글 패턴으로 본문 노드 변경 없이 디자이너가 직접 인스턴스에서 켜고 끔.
- **튜닝 노브 (각도 임계, DeadZone 등) 가 디자이너 손에 있는** 프로젝트: 함수 호출 노드의 핀 default 로 노출되어 컴파일 없이 조정.
- **튜토리얼 / 학습용 캐릭터** 가 있는 게임: Numpad +/- 로 Global Time Dilation 을 조정해 모션을 한 프레임씩 보여줄 수 있어 학습 / QA / 디자이너 검증에 유용.

### 적합하지 않은 경우

- **캐릭터가 항상 정면 (이동 방향) 만 바라보는** 게임: 1 방향 (Forward) 시퀀스만으로 충분. 4 방향 / 히스테리시스 불필요.
- **사이드 스크롤 / 탑다운 RPG** 같은 시점 고정 게임: 방향성 판정이 게임 코드 / UI 입력 단에서 결정됨.
- **시퀀스 자산이 Forward 1 종만 있어서** 4 방향 확장이 불가능한 프로젝트: 자산 확장 전에는 본 Step 도입의 효과가 미미.

### 권장 사양

| 항목 | 권장 |
|---|---|
| 시퀀스 자산 | 속도 모드 별 (Walk / Jog) 의 4 방향 (Forward / Backward / Right / Left) 시퀀스. 즉 최소 8 개 + Idle 1 개 |
| 디버그 키 입력 | Numpad +/- 같은 디버그 키를 정의 가능한 입력 시스템 (UE 의 `InputDebugKey` 권장. 개발 빌드 전용) |
| 엔진 | UE 5.x (`Calculate Direction` 엔진 함수 + `InputDebugKey`) |
| 추가 플러그인 | 없음 |
| 선행 Step | [Step 1](./LectureStep1.md), [Step 2](./LectureStep2.md) (Linked Anim Layer + Gait + LocomotionSM 2 상태) |
| 후속 Step 의존성 | 자연스러운 정지 / 출발 / 급선회까지 필요하면 Step 4 |
