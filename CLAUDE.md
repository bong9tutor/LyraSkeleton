...
| [docs/NotionPagePolicy.md](docs/NotionPagePolicy.md) | 본 저장소의 강의 / 분석 문서를 노션 페이지로 옮길 때 따르는 양식 / 구조 / 절차 규칙. **노션 페이지를 생성 / 수정하기 전 반드시 본 문서를 먼저 확인**. 생성 모드 (단일 페이지 / Step 별 / 심화) / 원본 선택 (`docs/Lectures` 우선, Announcements 와 섞지 않음) / Step 본문 템플릿 (결과 콜아웃 통합·접근성 라벨, "꼭 알아야 할 기능" 5개+ 시 분리, 빈 슬롯 제거) / 노션 양식 (H1~H4 토글 깊이, 콜아웃 텍스트 라벨, 큰 표 추가 접기, 번호 매김 ASCII 기본) / 링크 변환 / 페이지 생성 절차 (Claude MCP 도구명 / 환경 비종속 원칙 분리) / 사후 검증 체크리스트. |
