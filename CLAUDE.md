...
| `Plugins/Monolith/` (서브모듈, `tumourlove/monolith` v0.14.10 핀) | Claude 가 본 프로젝트 BP·AnimBP·데이터 자산을 분석할 때 사용하는 MCP 서버 플러그인. 사용법·능력·한계는 위 `Research_UE_Asset_Analyze.md` 참조. 부모 `.gitmodules` 에 `update = none` 이 설정되어 있으므로 클론 후 `git submodule update --init --checkout Plugins/Monolith` 한 줄로 본체만 받는다 (monolith upstream 의 nested wiki `.gitmodules` 누락 회피 - `--recurse-submodules` 자동 옵션도 이제 에러 없이 skip 됨, 상세 안내는 [README.md](README.md) 의 "1. 저장소 클론" 절). |
